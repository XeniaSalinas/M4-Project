...
D1 = readpfm('Data/new_view/disp0.pfm');
D2 = readpfm('Data/new_view/disp1.pfm');
[h, w, c] = size(im0);
D1out = D1;
D2out = D2;
D1 = round(D1);
...
Consistent1(:) = abs(D1(:) - D2(ind)) < 10;
ind = sub2ind(size(D1),I(:),min(J(:)+D2(:),size(D1,2)));
Consistent2(:) = abs(D1(ind) - D2(:)) < 10;
D1out(~Consistent1) = 0;
D2out(~Consistent2) = 0;
figure(1);imshow(D1out);
figure(2);imshow(D2out);
im0 = rgb2gray(im0);
im1 = rgb2gray(im1);
I_new = zeros(size(im0));
s=0.5;

for j = 1:w
    for i = 1:h
        if (D1out(i,j)~=0 & D2out(i,j)~=0 & D1out(i,j)<(j-1) & (j+D2(i,j))<w)
            p = floor((1-s)*[i,(j+D2(i,j))] + s*[i, j-D1(i,j)]);
            %I_new(p,2) =(1-s)*im0(i,j,2) + s*im1(i, floor(j-D1out(i,j)),2);
            I_new(p) =(1-s)*im0(i,(j+D2(i,j))) + s*im1(i, floor(j-D1(i,j)));
            %I_new(p,2) =(1-s)*im0(i,j,2) + s*im1(i, floor(j-D1out(i,j)),2);
            %I_new(p,3) =(1-s)*im0(i,j,3) + s*im1(i, floor(j-D1out(i,j)),3);
        
        elseif (D2out(i,j)==0)
            I_new(i,j) = im0(i, j);
        elseif (D1out(i,j)==0 & floor(j-D1(i,j))>1)
            p = floor([i, j-D1(i,j)]);
            I_new(p) = im1(i,floor(j-D1(i,j)));
        end
